...
int main(int argc, char** argv) {
    ...
    // Création des valeurs aléatoires générées (par la racine) à envoyer
        ...
        if (fils_gauche < n) {
            MPI_Send(msg, 2, MPI_INT, fils_gauche, 0, MPI_COMM_WORLD);
        }
        if (fils_droit < n) {
            MPI_Send(msg, 2, MPI_INT, fils_droit, 0, MPI_COMM_WORLD);
        }
    } 
    // Relais
    else {
        // Reception
        int pere = (id - 1) / 2;
        MPI_Recv(msg, 2, MPI_INT, pere, 0, MPI_COMM_WORLD, &status);
